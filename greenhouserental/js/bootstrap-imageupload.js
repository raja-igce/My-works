...
(function($) {
    ...
    function init(givenOptions) {
        ...
        var $fileTabButton = $imageupload.find('.panel-heading .btn1:eq(0)');
        var $browseFileButton = $fileTab.find('input[type="file"]');
        var $removeFileButton = $fileTab.find('.btn1:eq(1)');
        var $urlTab = $imageupload.find('.url-tab');
        var $urlTabButton = $imageupload.find('.panel-heading .btn1:eq(1)');
        var $submitUrlButton = $urlTab.find('.btn1:eq(0)');
        var $removeUrlButton = $urlTab.find('.btn1:eq(1)');
    ...
    function getImageThumbnailHtml(src) {
        return '<img src="' + src + '" alt="Image preview" class="thumbnail" style="width:100px; height:100px; max-width: ' + options.maxWidth + 'px; max-height: ' + options.maxHeight + 'px">';
    }
    ...
    function showFileTab($fileTab) {
        ...
        if (!$fileTabButton.hasClass('active')) {
            ...
            $imageupload.find('.panel-heading .btn1:eq(1)').removeClass('active');
    ...
    function resetFileTab($fileTab) {
        ...
        $fileTab.find('.btn1 span').html('<i class="fa fa-upload" aria-hidden="true"></i> Click here to Upload');
        $fileTab.find('.btn1:eq(1)').hide();
        $fileTab.find('input').val('');
    }

    function submitImageFile($fileTab) {
        var $browseFileButton = $fileTab.find('.btn1:eq(0)');
        var $removeFileButton = $fileTab.find('.btn1:eq(1)');
        var $fileInput = $browseFileButton.find('input');
        
        $fileTab.find('.alert').remove();
        $fileTab.find('img').remove();
        $browseFileButton.find('span').html('<i class="fa fa-upload" aria-hidden="true"></i> Click here to Upload');
        ...
        isValidImageFile(file, function(isValid, message) {
            if (isValid) {
                ...
                fileReader.onload = function(e) {
                    ...
                    $browseFileButton.find('span').html('<span class="glyphicon glyphicon-pencil btn1 btn-warning"></span>');
					
                    //$browseFileButton.find('span').addClass("btn1 btn btn-warning");
    ...
    function showUrlTab($urlTab) {
        ...
        if (!$urlTabButton.hasClass('active')) {
            ...
            $imageupload.find('.panel-heading .btn1:eq(0)').removeClass('active');
