...
class Cl_User
{
	...
	public function registration( array $data )
	{
		if( !empty( $data ) ){
			...
			$otp = mysqli_real_escape_string( $this->_con, $trimmed_data['otp'] );
			...
			{
			$passcode =	substr(mt_rand(10,9999999999),0,6);
			}
			$query = "INSERT INTO user (user_id, name, email, password, created, tearms, mobile, user_status,hash_code,acc_activate,otpcode) VALUES (NULL, '$name', '$email', '$password', CURRENT_TIMESTAMP, '$tearms', '$phone', '1', '$hash_code', '1', '$passcode')";
...
$ok1 = @mail($email_to, $email_subject, $email_message, $headers);
	...
	$msg1 = "Thank you for registering on our site. Your OTP Code is"." ".$passcode.".";		
		$msg = urlencode($msg1);
		...
		{ 
header('Location: mobileotp.php?otp='.urlencode(base64_encode($phone)));
        } 
		curl_close($ch);	
	...
	public function login( array $data )
	{
		...
		if( !empty( $data ) ){
			...
			$password = md5( $password );
			$query = "SELECT user_id, name, email, created, mobile, user_status, acc_activate FROM user where email = '$email' and password = '$password' ";
	...
	/* otp login */
		...
		public function otplogin( array $data )
	{
		$_SESSION['logged_in'] = false;
		if( !empty( $data ) ){
			...
			// escape variables for security
			$phone = mysqli_real_escape_string( $this->_con,  $trimmed_data['phone'] );
			$otppass = mysqli_real_escape_string( $this->_con,  $trimmed_data['otppass'] );
				
			if((!$phone) || (!$otppass) ) {
				throw new Exception( LOGIN_FIELDS_MISSING );
			}
			$password = md5( $password );
			$query = "SELECT user_id, name, email, created, mobile, user_status, otpsigin, acc_activate FROM user where mobile = '$phone' and otpsigin = '$otppass' ";
			$result = mysqli_query($this->_con, $query);
			$data = mysqli_fetch_assoc($result);
			...
			throw new Exception( USER_VERIFY );	
				...
				throw new Exception( LOGIN_FAILOTP );
			}
		} else{
			throw new Exception( LOGIN_FIELDS_MISSING );
		}
...
public function google_login( array $data )
	{
		...
		$query1 = "SELECT user_id, name, email, created, user_status FROM user where email = '$email' and social_id = '' ";
		$result1 = mysqli_query($this->_con, $query1);
		$count1 = mysqli_num_rows($result1);
		if($count1 == 1)
		{
		header('Location: userlogin.php?err='.urlencode(base64_encode(usblock)));	
		//return false;
		//throw new Exception(USER_BLOCK);
exit;		
		}
		...
		if( $count == 1){
			...
			$query = "INSERT INTO user (name, email, social_id, picture, created, user_status, acc_activate) VALUES ('$name', '$email', '$social_id', '$picture', CURRENT_TIMESTAMP,'1','2')";
			if(mysqli_query($this->_con, $query));
	...
	public function profile( array $data )
	{
		if( !empty( $data ) ){
			...
			if((!$c_username) || (!$c_phone)) {
				throw new Exception( FIELDS_MISSING );
			}
			
			$c_password = md5( $c_password );
			$query = "UPDATE user SET name = '$c_username', mobile = '$c_phone' WHERE user_id = '$user_id'";
			if(mysqli_query($this->_con, $query))
			{
            	self::logout();			
			return true;
