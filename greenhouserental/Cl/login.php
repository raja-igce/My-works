...
if(isset($session)){
	...
	$data['image'] = $image;
	if($user_obj->fb_login($data)){header('Location: dashboard.php');}
...
if ($client->getAccessToken()) {
	...
	try {
		if($user_obj->google_login( $user ))header('Location: dashboard.php');
		else header('Location: index.php');
	}catch (Exception $e) {
		$error = $e->getMessage();
	}
...
if(isset($_GET['type']) && $_GET['type'] == 'twitter' ){ 
	...
	if(( isset( $_SESSION['oauth_token'] ) ) ){
		...
		if( !empty( $content->id )){
			...
			try {
				if($user_obj->twitter_login($data))header('Location: dashboard.php');
			}catch (Exception $e) {
				$error = $e->getMessage();
			}
